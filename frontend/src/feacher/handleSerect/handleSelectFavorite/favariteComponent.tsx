...
// 新しく作成したお気に入りリポジトリ取得APIを直接叩いてモーダルUIでリスト表示する関数
export const showFavoriteReposModal = async (): Promise<Repo | null> => {
  try {
    // APIからお気に入りリポジトリを直接取得
    const response = await fetch('http://localhost:3030/api/db/repos/favorites');
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
    const favoriteRepos: Repo[] = await response.json();
  ...
  return await new Promise<Repo | null>(resolve => {
    ...
    console.error('お気に入りリポジトリの取得に失敗しました:', error);
    alert("お気に入りリポジトリの取得に失敗しました");
    return null;
  }
};
