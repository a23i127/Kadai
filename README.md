...
⭐️妥協点？悔しい実装
  ・データを取得して、保存し、dbをキャッシュとして活用しいていく実装の中で、各ファイルの中身(コードが書いてある部分)は、ファイルを開かないと、APIの都合上取得することができないため、開いていないファイルのキャッシュデータはnullとなってしまう。ので、キャッシュデータ取得をAPIよりも優先していた都合上,初めに開いてないファイルのキャッシュを取得した際、ファイルのコードを見ることができなった。ので、ファイルの中身を読み取る際はキャッシュよりもAPIで取得することを優先する実装に路線を変更して実装している。もっとここに時間をかけたかったが、制限時間の都合上妥協してしまった。
->本当はしたかった実装
  ・ファイルディレクトリ情報はキャッシュで取ってきて、ファイルを表示する際に、キャッシュにcontent情報がnullかをチェックして、nullならAPI叩いて、dbのキャッシュ情報を更新みたいな感じにしたかった。
⭐️改善点(反省)
  ・基盤を作るとこに時間がかかりすぎた。
...
・デバックが超大好きになったこと！
・デバックとAIを効率よく使うことで、時間をかなり短縮して実装できるってことが分かった。
