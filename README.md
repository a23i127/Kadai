...
⭐️妥協点？みたいなとこ
  ・データを取得して、保存し、dbをキャッシュとして活用しいていく実装の中で、各ファイルの中身(コードが書いてある部分)は、ファイルを開かないと、APIの都合上取得することができないため、開いていないファイルのキャッシュデータはnullとなってしまう。ので、キャッシュデータ取得をAPIよりも優先していた都合上,初めに開いてないファイルのキャッシュを取得した際、ファイルのコードを見ることができなった。
⭐️改善点(反省)
  ・基盤を作るとこに時間がかかりすぎた。
